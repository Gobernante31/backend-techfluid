...
export class VerificationRepository {
  // Accept the Worker Env so we can access D1 bindings
  ...
  async create(input: CreateValidationInput): Promise<KycValidation> {
    ...
    // Persist images directly into D1 as data URLs (base64) and save record.
    if (database) {
      await database
        .prepare(
...
function mapRow(row: D1Row): KycValidation {
  return {
    ...
    // Images are stored directly in D1 as data URLs (base64)
